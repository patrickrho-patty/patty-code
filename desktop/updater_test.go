...
func TestUpdateSiblingNamesCoverEveryReplacedEntryPoint(t *testing.T) {
	...
	for _, want := range []string{"patty-guard.exe", "patty-launcher.exe", "patty-update-helper.exe", "patty-cli.exe", "Patty Code.exe"} {
		if !strings.Contains(windows, want) {
			t.Errorf("Windows release unit omits %q: %q", want, windows)
		}
	}
	if strings.Contains(windows, "patty.exe") {
		t.Fatalf("Windows release unit reintroduces the case-only CLI/launcher collision: %q", windows)
	}
	if got := updateSiblingNames("linux"); len(got) != 2 || got[0] != "patty-guard" || got[1] != "patty" {
...
func TestLegacyChannelsSelectOfficialPointers(t *testing.T) {
	...
	if got := downloadPage("preview"); got != "https://patty.io/?download=desktop#start" {
		t.Errorf("legacy preview download page = %q", got)
	}
	if got := manifestDownloadPage("preview", "https://patty.io/?channel=preview&download=desktop#start"); got != "https://patty.io/?download=desktop#start" {
...
func TestDesktopManifestValidation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mutate: func(m *update.Manifest) {
				...
				asset.URL = strings.Replace(asset.URL, "Patty Code-", "Other-", 1)
		...
		},
		{
			...
			mutate: func(m *update.Manifest) {
				asset := m.Platforms["darwin-arm64"]
				asset.URL = strings.Replace(asset.URL, "dl.patty.io", "example.com", 1)
				asset.Sig = asset.URL + ".minisig"
				m.Platforms["darwin-arm64"] = asset
			},
		},
		...
		},
		{
			...
			mutate: func(m *update.Manifest) {
				m.DownloadPage = "https://patty.io/?channel=stable&download=desktop#start"
			},
		},
	}
	...
	t.Run("unified GitHub release base", func(t *testing.T) {
		...
		newBase := "https://github.com/pattycorp/PattyCode/releases/download/v1.19.0/"
		for key, asset := range manifest.Platforms {
	...
	t.Run("mixed official bases", func(t *testing.T) {
		...
		asset.URL = strings.Replace(
			...
			"https://github.com/pattycorp/PattyCode/releases/download/desktop-v1.18.0/",
			1,
		)
		asset.Sig = asset.URL + ".minisig"
...
func TestValidateUpdateRedirect(t *testing.T) {
	tests := []struct {
		...
		{name: "Patty Code first-party redirect", target: "https://dl.patty.io/file"},
		...
		{name: "Patty Code suffix spoof", target: "https://dl.patty.io.evil.invalid/file", wantError: true},
		{name: "GitHub suffix spoof", target: "https://release-assets.githubusercontent.com.evil.invalid/file", wantError: true},
		{name: "explicit port", target: "https://dl.patty.io:443/file", wantError: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestSaveCachedUpdateMarksEvaluateDownloaded(t *testing.T) {
	...
	asset := update.Asset{
		URL:    "https://dl.patty-code.io/desktop-v9.9.9/Patty Code-linux-amd64.tar.gz",
		Size:   int64(len(data)),
...
func TestDebCacheRequiresSignatureAndRejectsTarballReuse(t *testing.T) {
	...
	data := []byte("deb-bytes")
	asset := update.Asset{
		URL:    "https://dl.patty-code.io/desktop-v9.9.9/Patty Code-linux-amd64.deb",
		Size:   int64(len(data)),
		SHA256: sha256Hex(data),
	}
...
func TestExtractBinary(t *testing.T) {
	...
	tw := tar.NewWriter(gz)
	files := map[string][]byte{"README": []byte("ignore me"), "patty-desktop": want}
	...
	got, err := extractBinary(buf.Bytes(), "patty-desktop")
	if err != nil {
...
func TestExtractLinuxReleaseUnitRejectsAmbiguousMembers(t *testing.T) {
	...
	headers := []tar.Header{
		base("patty-desktop", []byte("desktop")),
		base("patty-guard", []byte("guard")),
		base("patty", []byte("cli")),
	}
	bodies := [][]byte{[]byte("desktop"), []byte("guard"), []byte("cli")}
	if got, err := extractLinuxReleaseUnit(makeArchive(t, headers, bodies)); err != nil ||
		string(got["patty-desktop"]) != "desktop" {
		t.Fatalf("complete release extraction = %v, %q", err, got["patty-desktop"])
	}

	duplicateHeaders := append(append([]tar.Header(nil), headers...), base("nested/patty", []byte("duplicate")))
	...
	nonRegular[1] = tar.Header{Name: "patty-guard", Typeflag: tar.TypeSymlink, Linkname: "outside"}
...
func TestApplyLinuxVersionedActivatesWithoutPersistingGuard(t *testing.T) {
	...
	tw := tar.NewWriter(gz)
	for _, name := range []string{"patty-desktop", "patty-guard", "patty"} {
		body := []byte("new-" + name)
		if err := tw.WriteHeader(&tar.Header{Name: name, Mode: 0o755, Size: int64(len(body)), Typeflag: tar.TypeReg}); err != nil {
			t.Fatal(err)
		}
	...
	if err != nil || string(data) != "new-patty-desktop" {
		t.Fatalf("active desktop=%q err=%v", data, err)
	}
	for _, guardPath := range []string{
		filepath.Join(root, "patty-guard"),
		filepath.Join(root, "versions", "v1.20.1", "patty-guard"),
	} {
...
func TestApplyLinuxHoldsReleaseUnitLockDuringReplace(t *testing.T) {
	...
	exe := filepath.Join(dir, "patty-desktop")
