...
####
## Patty Code per-user NSIS installer.
##
...
##      forces the user back to %LOCALAPPDATA%\Programs\Patty Code even if they had
##      moved the install to a different drive (e.g. D:\Tools\Patty Code); the
...
!if /FileExists "patty_host.nsh"
!include "patty_host.nsh"
!endif
!ifndef PATTY_CODE_UNINST_FINALIZE
!define PATTY_CODE_UNINST_FINALIZE 'cmd.exe /C copy /Y "%1" "patty-code-uninstall.exe" >NUL'
!endif
...
!define MUI_PAGE_CUSTOMFUNCTION_SHOW patty.showUpdateProgress
...
## The second pass provides ARG_PATTY_CODE_SIGNED_UNINSTALLER and embeds that
## signed binary instead of generating another unsigned uninstaller.
!ifndef ARG_PATTY_CODE_SIGNED_UNINSTALLER
!uninstfinalize '${PATTY_CODE_UNINST_FINALIZE}'
!endif
#!finalize 'signtool --file "%1"'

Name "${INFO_PRODUCTNAME}"
OutFile "..\..\bin\${INFO_PROJECTNAME}-${ARCH}-installer.exe" # Name of the installer's file.
!define PATTY_DEFAULT_INSTALLDIR "$LOCALAPPDATA\Programs\${INFO_PRODUCTNAME}"
...
!define PATTY_UNLOCK_RETRIES 60
...
InstallDir "${PATTY_DEFAULT_INSTALLDIR}" # Per-user install location (no admin rights required).
...
####
!macro patty.writeUninstaller
    !ifdef ARG_PATTY_CODE_SIGNED_UNINSTALLER
    File "/oname=uninstall.exe" "${ARG_PATTY_CODE_SIGNED_UNINSTALLER}"
    !else
    ...
    !if /FileExists "${PATTY_LAUNCHER}"
    WriteRegStr HKCU "${UNINST_KEY}" "DisplayIcon" "$INSTDIR\${PATTY_LAUNCHER}"
    !else
    ...
    # user back to %LOCALAPPDATA%\Programs\Patty Code even if they had moved
    # the install to a different drive (e.g. D:\Tools\Patty Code). The auto-
...
!macro patty.deleteLegacyInstallerStateIfOwned
    StrCpy $1 "1"
    ClearErrors
    ReadRegStr $0 HKCU "${PATTY_LEGACY_PRODUCT_KEY}" ""
    ${If} $0 == "$INSTDIR"
        ClearErrors
    ...
    ${If} $1 == "1"
        ClearErrors
        ReadRegStr $0 HKCU "${PATTY_LEGACY_UNINST_KEY}" "InstallLocation"
        ${If} $0 == "$INSTDIR"
            DeleteRegKey HKCU "${PATTY_LEGACY_UNINST_KEY}"
        ${ElseIf} $0 == "$\"$INSTDIR$\""
            DeleteRegKey HKCU "${PATTY_LEGACY_UNINST_KEY}"
        ${Else}
            ClearErrors
            ReadRegStr $0 HKCU "${PATTY_LEGACY_UNINST_KEY}" "UninstallString"
...
Function .onInit
   ...
   ; The helper passes /PATTYCODEUPDATE=1 and a final /D=<current directory>.
   ...
   ; can relaunch Patty Code. A normal manual installer keeps the full wizard.
   StrCpy $PattyCodeUpdateMode "0"
   StrCpy $PattyCodeStageMode "0"
   ${GetParameters} $R0
   ClearErrors
   ${GetOptions} $R0 "/PATTYCODEUPDATE=" $R1
   IfErrors patty_update_mode_done
   StrCmp $R1 "1" 0 patty_update_mode_done
   StrCpy $PattyCodeUpdateMode "1"

patty_update_mode_done:
   ClearErrors
   ${GetOptions} $R0 "/PATTYCODESTAGE=" $R2
   IfErrors patty_stage_mode_done
   StrCmp $R2 "1" 0 patty_stage_mode_done
   StrCpy $PattyCodeStageMode "1"

patty_stage_mode_done:
...
legacy_location:
   ...
   ; directory with surrounding quotes (for example "D:\Patty Code"). Reuse it
...
fallback:
   StrCpy $INSTDIR "${PATTY_DEFAULT_INSTALLDIR}"
done:
FunctionEnd
...
patty_show_setup_page:
FunctionEnd

Function patty.showUpdateProgress
   StrCmp $PattyCodeUpdateMode "1" 0 patty_update_progress_done
   !insertmacro MUI_HEADER_TEXT "$(pattyUpdateTitle)" "$(pattyUpdateSubtitle)"
   SetDetailsView hide
   SetAutoClose true
   BringToFront

patty_update_progress_done:
FunctionEnd
...
check_guard:
   IfFileExists "$INSTDIR\${PATTY_GUARD}" 0 check_launcher
   ClearErrors
...
check_launcher:
	IfFileExists "$INSTDIR\${PATTY_LAUNCHER}" 0 check_cli
	ClearErrors
...
check_cli:
	IfFileExists "$INSTDIR\${PATTY_CLI}" 0 check_portable_entry
	ClearErrors
...
check_portable_entry:
   IfFileExists "$INSTDIR\${PATTY_PORTABLE_ENTRY}" 0 done
   ClearErrors
...
locked:
   IntOp $0 $0 + 1
   IntCmp $0 ${PATTY_UNLOCK_RETRIES} failed 0 0
   Sleep 1000
   Goto retry
...
interactive:
   MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION "Patty Code is still running. Close Patty Code, then click Retry to continue the installation." IDRETRY retry IDCANCEL abort
   Goto retry
...
abort:
   Abort "Patty Code is still running. Close Patty Code and run the installer again."
...
Section
    ...
    ; /PATTYCODESTAGE=1: flat six-member payload for 1.18–1.19.1 helpers (and
    ...
    ; a normal install.
    StrCmp $PattyCodeStageMode "1" patty_stage_payload
    !insertmacro wails.webview2runtime
    Call patty.waitForExecutableUnlock
    Goto patty_normal_install

patty_stage_payload:
    SetOutPath $INSTDIR
    !if /FileExists "${PATTY_PAYLOAD_MANIFEST}"
    File "/oname=${PATTY_PAYLOAD_MANIFEST}" "${PATTY_PAYLOAD_MANIFEST}"
    !endif
    !if /FileExists "${PATTY_PAYLOAD_SIGNATURE}"
    File "/oname=${PATTY_PAYLOAD_SIGNATURE}" "${PATTY_PAYLOAD_SIGNATURE}"
    !endif
    !insertmacro wails.files
    !if /FileExists "${PATTY_UPDATE_HELPER}"
    File "/oname=${PATTY_UPDATE_HELPER}" "${PATTY_UPDATE_HELPER}"
    !endif
    !if /FileExists "${PATTY_GUARD}"
    File "/oname=${PATTY_GUARD}" "${PATTY_GUARD}"
    !endif
    !if /FileExists "${PATTY_LAUNCHER}"
    File "/oname=${PATTY_LAUNCHER}" "${PATTY_LAUNCHER}"
    !endif
    !if /FileExists "${PATTY_CLI}"
    File "/oname=${PATTY_CLI}" "${PATTY_CLI}"
    !endif
    Goto patty_section_done

patty_normal_install:
    ...
    File "/oname=${PATTY_UPDATE_HELPER}" "${PATTY_UPDATE_HELPER}"
    !else
    !warning "${PATTY_UPDATE_HELPER} was not found; Windows auto-update will fail safely until the helper is installed."
    !endif
    !if /FileExists "${PATTY_CLI}"
    File "/oname=${PATTY_CLI}" "${PATTY_CLI}"
    !else
    !warning "${PATTY_CLI} was not found; remote upload installation will be unavailable."
    !endif
    !if /FileExists "${PATTY_LAUNCHER}"
    File "/oname=${PATTY_LAUNCHER}" "${PATTY_LAUNCHER}"
    !endif
    ...
    !else
    !error "${PATTY_GUARD} was not found; normal installs require the signed layout activator."
    !endif
    DetailPrint "Patty Code layout activator output:"
    nsExec::ExecToLog /OEM '"$PLUGINSDIR\${PATTY_LAYOUT_INSTALLER}" --install-root "$INSTDIR" --version "v${INFO_PRODUCTVERSION}" --activate-staging "$R9" --no-relaunch'
    Pop $0
    StrCmp $0 "0" patty_layout_activated
    DetailPrint "Patty Code layout activation failed with exit code $0; the previous version remains active."
    RMDir /r "$R9"
    SetErrorLevel 1
    Abort "Patty Code could not activate the verified release. The previous version was left unchanged."

patty_layout_activated:
    RMDir /r "$R9"
    SetOutPath "$INSTDIR"

    ; Remove flat leftovers from prior 1.18–1.19 installs when overwriting.
    Delete "$INSTDIR\${PRODUCT_EXECUTABLE}"
    Delete "$INSTDIR\${PATTY_GUARD}"
    Delete "$INSTDIR\${PATTY_UPDATE_HELPER}"

    !if /FileExists "${PATTY_LAUNCHER}"
    ; Keep both target and icon on the stable launcher. Pointing IconLocation at
    ; versions\vX\patty-desktop.exe leaves a blank shortcut as soon as version
    ; retention removes that directory after a later update.
    CreateShortcut "$SMPROGRAMS\${INFO_PRODUCTNAME}.lnk" "$INSTDIR\${PATTY_LAUNCHER}" "" "$INSTDIR\${PATTY_LAUNCHER}" 0
    CreateShortCut "$DESKTOP\${INFO_PRODUCTNAME}.lnk" "$INSTDIR\${PATTY_LAUNCHER}" "" "$INSTDIR\${PATTY_LAUNCHER}" 0
    !else
    ...
    !insertmacro patty.writeUninstaller
...
Section "uninstall"
    ...
    Delete "$INSTDIR\${PATTY_UPDATE_HELPER}"
    Delete "$INSTDIR\${PATTY_GUARD}"
    Delete "$INSTDIR\${PATTY_LAUNCHER}"
    Delete "$INSTDIR\${PATTY_CLI}"
    Delete "$INSTDIR\${PATTY_PORTABLE_ENTRY}"
    ...
    !insertmacro patty.deleteUninstaller
    !insertmacro patty.deleteLegacyInstallerStateIfOwned
