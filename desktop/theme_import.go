...
// plugin theme loading share it so the container contract cannot drift.
func scanThemeZipEntries(zr *zip.Reader) (manifestEntry *zip.File, imageEntries map[string]*zip.File, err error) {
	...
	for _, zf := range zr.File {
		...
		if name == "" {
			return nil, nil, fmt.Errorf("theme package contains an empty or unsafe path")
		}
		if strings.Contains(name, "/") || strings.Contains(name, `\`) {
			return nil, nil, fmt.Errorf("theme package may only contain root-level files (got %q)", zf.Name)
		}
		if _, dup := seen[strings.ToLower(name)]; dup {
			return nil, nil, fmt.Errorf("theme package contains duplicate entry %q", name)
		}
		seen[strings.ToLower(name)] = struct{}{}

		if zf.FileInfo().IsDir() {
			return nil, nil, fmt.Errorf("theme package must not contain directories")
		}
		// Detect symlink-like mode bits when present.
		if zf.Mode()&os.ModeSymlink != 0 {
			return nil, nil, fmt.Errorf("theme package must not contain symlinks")
		}
		if zf.UncompressedSize64 > themePackMaxImageBytes && !strings.EqualFold(name, themePackManifestName) {
			return nil, nil, fmt.Errorf("theme package entry %q is too large", name)
		}
		if zf.UncompressedSize64 > themePackMaxManifest && strings.EqualFold(name, themePackManifestName) {
			return nil, nil, fmt.Errorf("theme manifest is too large")
		}
		...
		if themePackImageRe.MatchString(name) {
			if len(imageEntries) >= 2 {
				return nil, nil, fmt.Errorf("theme package may contain at most two scene images")
			}
			imageEntries[strings.ToLower(name)] = zf
			continue
		}
		return nil, nil, fmt.Errorf("theme package contains disallowed file %q", name)
	}

	if manifestEntry == nil {
		return nil, nil, fmt.Errorf("theme package missing %s", themePackManifestName)
	}
	return manifestEntry, imageEntries, nil
}

// themeZipExpectedImages maps the manifest-declared scene images (lowercase
// name -> manifest name) that the ZIP must carry.
func themeZipExpectedImages(m *ThemePackManifest) map[string]string {
...
// checkThemeZipImages verifies the manifest references exactly the images the
// ZIP carries — no missing, no extras.
func checkThemeZipImages(m *ThemePackManifest, imageEntries map[string]*zip.File) error {
	expectedImages := themeZipExpectedImages(m)
	for key, manifestName := range expectedImages {
		if imageEntries[key] == nil {
			return fmt.Errorf("manifest references scene image %q but ZIP has none", manifestName)
		}
	}
	for key := range imageEntries {
		if _, ok := expectedImages[key]; !ok {
			return fmt.Errorf("ZIP contains scene image not referenced by manifest")
		}
	}
	return nil
}
