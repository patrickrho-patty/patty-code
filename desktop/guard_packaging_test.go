...
func TestVerifyWindowsPortableVersionedLayout(t *testing.T) {
	...
	writePortableFixture(t, good, "patty-code-launcher.exe", "launcher")
	writePortableFixture(t, good, "PatCode.exe", "launcher")
	writePortableFixture(t, good, "patcode-cli.exe", "cli")
...
func TestDesktopPackagesPreserveNativePlatformLaunchers(t *testing.T) {
	...
	for _, want := range []string{
		`CLINAME="patcode"`,
		`WINDOWS_CLINAME="patcode-cli"`,
		`./cmd/patcode`,
		`./cmd/patty-code-legacy-migrator`,
		`./cmd/patty-code-launcher`,
		...
		`-H windowsgui`,
		`stamp_windows_executable "$guard_out" "Patty Code Legacy Migrator"`,
		`stamp_windows_executable "$launcher_out" "Patty Code Launcher"`,
		`stamp_windows_executable "build/windows/installer/$UPDATE_HELPER" "Patty Code Update Helper"`,
	...
	launcherStamp := strings.Index(build, `stamp_windows_executable "$launcher_out" "Patty Code Launcher"`)
	...
	if strings.Contains(build, `"$staging/$CLINAME.exe"`) {
		t.Fatal("Windows package must not collide patcode.exe with the PatCode.exe launcher")
	}
	...
	for _, want := range []string{
		`dpkg-deb --field "$deb_path" Package | grep -x 'patty-desktop'`,
		`usr/lib/patty/patty-code-update-helper`,
		`usr/share/polkit-1/actions/io.patty-code.desktop.update.policy`,
	} {
		if !strings.Contains(build, want) {
	...
	desktopEntry, err := os.ReadFile("build/linux/patty-code.desktop")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(desktopEntry), "Exec=patty-code-launcher") || strings.Contains(string(desktopEntry), "patty-code-guard") || strings.Contains(string(desktopEntry), "patty-guard") {
	...
	if !strings.Contains(nfpm, "dst: /usr/bin/patty-code-launcher") || strings.Contains(nfpm, "dst: /usr/bin/patty-code-guard") || strings.Contains(nfpm, "dst: /usr/bin/patty-guard") {
	...
	if !strings.Contains(nfpm, "dst: /usr/share/applications/patty-code.desktop") {
		t.Fatal("Linux deb must install the patty desktop entry")
	}
	...
	if strings.Contains(windows, `ExecWait '"$PLUGINSDIR\${PATTY_LAYOUT_INSTALLER}"`) {
		t.Fatal("Windows installer must not discard layout activator stdout/stderr")
	}
	if strings.Contains(windows, `CreateShortcut "$SMPROGRAMS\${INFO_PRODUCTNAME}.lnk" "$INSTDIR\${PATTY_LAUNCHER}" "" "$INSTDIR\versions\v${INFO_PRODUCTVERSION}\${PRODUCT_EXECUTABLE}" 0`) ||
		strings.Contains(windows, `CreateShortCut "$DESKTOP\${INFO_PRODUCTNAME}.lnk" "$INSTDIR\${PATTY_LAUNCHER}" "" "$INSTDIR\versions\v${INFO_PRODUCTVERSION}\${PRODUCT_EXECUTABLE}" 0`) {
