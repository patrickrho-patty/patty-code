...
const DM = "__PATTY_MATH_DISPLAY__";
const IM = "__PATTY_MATH_INLINE__";
const LB = "__PATTY_LATEX_LINEBREAK__";
const ED_BASE = "PATTYESCAPEDDOLLAR";
const INLINE_SOURCE_PREFIX = "\\pattyInternalSourceV1{";
const INLINE_RENDER_SOURCE_PREFIX = "\\pattyInternalRenderV1";
const INLINE_RENDER_SOURCE_RE = /\\pattyInternalRenderV1\{([^}]*)\}\{([^}]*)\}/g;
...
function unusedPlaceholderPrefix(s: string): string {
  let prefix = "__PATTY_PROTECTED_CODE__";
  let n = 0;
  while (s.includes(prefix)) {
    n += 1;
    prefix = `__PATTY_PROTECTED_CODE_${n}__`;
  }
  return prefix;
}
