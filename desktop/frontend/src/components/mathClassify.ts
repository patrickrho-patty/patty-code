...
const CURRENCY_BEFORE =
  /(?:\b(?:costs?|prices?|priced|pay|paid|worth|fees?|budget|salary|total|amount|spend|spent|save|saved|charge|charged|buy|bought|sell|sold|usd|dollars?)\b)\s*(?:(?:is|was|are|were|of|at)\s*|[:=]\s*)?$/i;

const CURRENCY_AFTER =
  /^\s*(?:usd\b|dollars?\b|(?:in\s+)?cash\b|each\b|per\b|\/\s*(?:day|week|month|year)\b)/i;
