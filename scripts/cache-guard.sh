...
set +e
PATTY_CODE_RELEASE_CACHE_GUARD=1 go test ./internal/agent -run '^TestReleaseCacheHitGuard$' -v -count=1 2>&1 | tee "$tmp"
status=${PIPESTATUS[0]}
set -e
...
while IFS= read -r line; do
  ...
  echo "::warning title=Patty Code cache guard::$msg"
