// Dashboard authorization. Identity comes from id.patty.io (the shared
...
// The id.patty.io session cookie is scoped to `.patty.io`, so the browser
...
function idOrigin(env: Env): string {
  return (env.ID_ORIGIN ?? "https://id.patty.io").replace(/\/$/, "");
}

function appOrigin(env: Env): string {
  return (env.APP_ORIGIN ?? "https://patty-code.io").replace(/\/$/, "");
}
...
async function identityCacheKey(token: string): Promise<Request> {
  ...
  return new Request(`https://id-cache.patty.io/${hex}`);
}
...
// Ends the shared id.patty.io session (best-effort) and returns a Set-Cookie
...
export async function sharedLogout(request: Request, env: Env): Promise<string> {
  ...
  return `${SHARED_COOKIE}=; HttpOnly; Secure; SameSite=Lax; Path=/; Domain=.patty.io; Max-Age=0`;
}
