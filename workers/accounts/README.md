...
The account service for `patty-code.io`: email/password sign-up, email verification,
sessions, password reset, and public profiles. A Cloudflare Worker (Hono) backed
by D1. Runs at `id.patty.io`, separate from the internal crash dashboard.
...
```sh
wrangler d1 create patty-code-accounts        # paste database_id into wrangler.toml
wrangler d1 migrations apply patty-code-accounts --remote
wrangler secret put SESSION_PEPPER
wrangler secret put RESEND_API_KEY          # if EMAIL_PROVIDER=resend
wrangler deploy
```

The `id.patty.io` custom domain route is declared in `wrangler.toml`; point the
