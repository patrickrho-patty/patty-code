...
| Branch | `v1` (legacy maintenance line) | `pattycode` (active Patty Code line) |
...
```sh
git clone https://github.com/patrickrho-patty/patty-code
cd patty-code && make build                                 # -> bin/patcode(.exe)
```
...
See [Context Engine v2](../reference/SESSION_MEMORY_RETRIEVAL.md) for the full precedence,
...
Questions? Open a [Discussion](https://github.com/patrickrho-patty/patty-code/discussions).
