...
> (data types, registries, package layout, roadmap), see the **[Spec](../reference/SPEC.md)**.
...
- [Task contracts and pause policy](../reference/TASK_CONTRACT.md)
...
For the full schema and every field's contract, see [`SPEC.md` §5](../reference/SPEC.md#5-configuration-toml).
...
content-free event counters to the vendor telemetry endpoint. Configure the
...
can still reduce cache reuse for that turn. The desktop hook schema and loading
model are documented in the desktop hooks section above.
...
[Capability diagnostics](../reference/CAPABILITY_DIAGNOSTICS.md)). `/new` starts a new
...
See [Subagent profiles](../reference/SUBAGENT_PROFILES.md) for the complete CLI reference,
...
[`Context Engine v2`](../reference/SESSION_MEMORY_RETRIEVAL.md).
...
[task contract](../reference/TASK_CONTRACT.md): Context, Request, Output format,
...
[`SPEC.md` §3.5](../reference/SPEC.md#35-two-model-collaboration-coordinator).
