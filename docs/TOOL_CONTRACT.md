...
<a href="./TOOL_CONTRACT.ko-KR.md">(Korean)</a>

This document records the provider-visible contract for Patty Code compile-time built-in tools. It is generated from the same canonical schema path used by the runtime registry.
...
In a default full-token boot, Patty Code sends the built-in tools above plus the
...
In token economy mode, Patty Code starts with nine tools: four direct coding tools,
