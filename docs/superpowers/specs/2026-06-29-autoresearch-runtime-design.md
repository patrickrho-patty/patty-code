...
Patty Code already has Goal mode and AutoResearch instructions. When a goal looks
long-running, `activeGoalBlock` injects an AutoResearch protocol that asks the
model to create `.patty/autoresearch/<task-id>/` and maintain files such as
...
- No network, publish, payment, credential, or destructive-operation bypass.
  Existing Patty Code gates still apply.
...
2. If AutoResearch is on, the controller creates a task unless the goal contains
   an explicit `.patty/autoresearch/<task-id>/` path.
...
- Explicit: a goal or prompt includes `.patty/autoresearch/<task-id>/`.
...
`AutoResearchOpenTask` opens `.patty/autoresearch/<task-id>/` in the
...
- Resume: starts or continues `/goal --research .patty/autoresearch/<task-id>/`
...
- explicit `.patty/autoresearch/<task-id>/` resumes existing state
...
No existing `.patty/autoresearch` task should be deleted or rewritten without
