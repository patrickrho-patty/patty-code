...
The Ask / Auto / Yolo control under the desktop composer sets how Patty Code handles tool permission approvals. All three modes stay visible so you can switch directly without relying on a shortcut or settings page.

Tool permission is independent of collaboration mode:

- **Collaboration / runtime mode** decides how Patty Code advances the task (lightweight, balanced, or delivery-first).
...
Ask is the most conservative tool-permission mode. When Patty Code needs approval for a tool call, an approval card appears so you can allow once, allow for the session, always allow, or deny.
...
Headless `patcode run` has no approval card to answer. Its default Ask posture
...
- When entered via shortcut, Patty Code remembers the previous Ask/Auto baseline and restores it on the next toggle.
