...
with `runAs: subagent`: Patty Code starts an isolated child agent, gives it the
...
```bash
patcode subagent create reviewer \
...
`.patty/skills/<name>/SKILL.md`; global profiles are stored under the
Patty Code home Skill directory described in
[Configuration paths](../guides/CONFIG_PATHS.md).
...
printf '%s\n' 'Review the task and report only actionable findings.' | \
  patcode subagent create reviewer --description "Code reviewer"
```

Names may contain letters, digits, `_`, `-`, and `.`. Patty Code refuses a name
...
patcode subagent try reviewer "review the current diff"
...
git diff | patcode subagent run reviewer --max-steps 20
```
...
apply. Ordinary `patcode run` remains a plain one-shot task entry point and
does not implicitly interpret `/<profile>` syntax.

## Manage profiles

```text
patcode subagent list [--dir PATH]
patcode subagent create <name> --description TEXT (--prompt TEXT | --prompt-file PATH)
  [--scope project|global] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
patcode subagent edit <name> [--description TEXT]
  [--prompt TEXT | --prompt-file PATH] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
patcode subagent delete <name> --yes [--dir PATH]
patcode subagent try <name> [--model REF] [--max-steps N] [--dir PATH] <task>
patcode subagent run <name> [--model REF] [--max-steps N] [--dir PATH] <task>
```
...
```bash
patcode subagent edit reviewer --model= --effort= --tools= --color=
```
...
Profiles created in desktop settings and with `patcode subagent create` share
...
`patcode subagent list`, the current `--dir`, and `skills.disabled_skills`. If
...
invalid effort levels are rejected when Patty Code resolves the effective model.
