...
func InstalledNames(pattyHome string) ([]string, error) {
...
func InstalledListText(pattyHome string) (string, error) {
	st, err := LoadState(pattyHome)
	if err != nil {
		return "", err
	}
	if len(st.Plugins) == 0 {
		return "plugins: none installed\ninstall: patcode plugin install <source> --yes, or use Settings -> Plugins", nil
	}
	...
	for _, p := range st.Plugins {
		...
		counts := pluginCapabilityText(pattyHome, p)
		if summary != "" && counts != "" {
...
func InstalledShowText(pattyHome, name string) (string, error) {
	p, ok, err := FindInstalled(pattyHome, name)
	if err != nil {
		return "", err
	}
	if !ok {
	...
	root := ResolveRoot(pattyHome, p.Root)
...
func FindInstalled(pattyHome, name string) (InstalledPlugin, bool, error) {
	st, err := LoadState(pattyHome)
	if err != nil {
...
func pluginCapabilityText(pattyHome string, p InstalledPlugin) string {
	root := ResolveRoot(pattyHome, p.Root)
...
func RuntimeTrustText(rt *RuntimeSpec) string {
	...
	b.WriteString("  risk: the runtime process runs inside Patty Code — it can read the full session and environment, bypass permissions, and operate this machine directly.\n")
	return b.String()
}
