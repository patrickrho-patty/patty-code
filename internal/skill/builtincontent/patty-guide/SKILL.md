---
name: patty-guide
description: "Troubleshoot and configure Patty Code capabilities: Skills (project/custom/global/builtin priority, discovery dirs), Commands (override order, /dir:file naming), Hooks (11 events, automatic project loading, matchers, timeouts), MCP (patty.toml + .mcp.json + plugin packages, auto_start), plugin packages (native/Codex/Claude manifests), and AGENTS.md / instruction docs. Use when the user asks how to configure, debug missing skills/commands/hooks/MCP/plugins, or diagnose capability loading."
runAs: inline
---

# Patty Code self-diagnostics guide
...
```bash
patcode doctor capabilities --live --timeout 5s --json
```
...
1. **project** — `<workspace>/{.patty,.agents,.agent,.claude}/skills/`
2. **custom** — `[skills].paths` (and plugin package skill roots)
3. **global** — `<Patty Code home>/skills` and home convention dirs
...
Discovery conventions: `.patty`, `.agents`, `.agent`, `.claude` (see `config.ConventionDirs`). Layouts: `<name>/SKILL.md` or flat `<name>.md` (Claude flat files need skill frontmatter).

### Checks

| Entry | How |
| --- | --- |
| CLI | `patcode doctor capabilities` → Skills section |
| Desktop | Settings → Skills; Settings → Diagnostics |
| Agent | `/skill` list, `/patty-guide`, `run_skill` |
...
1. `patcode doctor capabilities --json` → Skills
...
`config.CommandDirsForRoot`: home convention commands → Patty Code home commands → project convention commands. **Later directory overrides earlier** on name clash (`command.Load`).
...
- Project: `<workspace>/.patty/settings.json` — loaded automatically
- Plugin packages: installed enabled packages
- Global: `<Patty Code home>/settings.json` (always)
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Project hooks silent | Wrong workspace / restart required | Confirm the project path and restart Patty Code after saving |
...
- Native: `patty-plugin.json`
- Codex: `.codex-plugin/plugin.json`
- Claude: `.claude-plugin/plugin.json` (+ limited Claude compatibility paths)

State: `<Patty Code home>/plugin-packages.json`. Disabled packages do not contribute skills/hooks/MCP.

Unmapped Claude-only features may appear as compatibility warnings — Patty Code does not invent support.

### Checks

`patcode plugin doctor <name>`, Settings → Plugins, Diagnostics → Plugins.
...
## Instructions (AGENTS.md / PATTY.md)

### Load order (ascending specificity)

User global docs → ancestor chain → project docs → project-local (`*.local.md`).

Recognized names: `PATTY.md`, `AGENTS.md`, `CLAUDE.md` (and `*.local.md` variants). Multiple files in one directory can load; symlink identity is deduped.
