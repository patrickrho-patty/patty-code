...
// buildExamplePlugin compiles cmd/patty-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "patty-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "patty/cmd/patty-plugin-example").CombinedOutput()
	if err != nil {
