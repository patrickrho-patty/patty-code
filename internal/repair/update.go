...
func lockPendingUpdateStrict() (func(), error) {
	...
	if path == "" {
		return nil, fmt.Errorf("pending update: Patty Code state directory is unavailable")
	}
...
func PrepareFileUpdate(fromVersion, toVersion, targetPath string, siblingPaths ...string) (*UpdateTransaction, error) {
	...
	root := config.MemoryUserDir()
	if root == "" {
		return nil, fmt.Errorf("prepare update: Patty Code state directory is unavailable")
	}
...
func PrepareAppBundleUpdateHandoff(fromVersion, toVersion, appPath, backupPath, stagedAppPath, stagingPath string, ownerPID int) (*UpdateTransaction, error) {
	...
	// An uncooperative writer is not covered by patty's mutation lock. Recheck
...
func newAppBundleUpdateTransaction(fromVersion, toVersion, appPath, backupPath string) (*UpdateTransaction, error) {
	...
	if !strings.HasSuffix(strings.ToLower(tx.TargetPath), ".app") ||
		tx.BackupPath != tx.TargetPath+".patty-update-backup" {
		return nil, fmt.Errorf("prepare update: invalid macOS bundle paths")
	}
	return tx, nil
}
...
func PublishClaimedFileUpdateMemberExact(
	...
	if !member.MissingBefore {
		...
		retained = member.TargetPath + ".patty-update-aside-" + transactionID[:16]
...
func stageFileUpdateContent(targetPath string, content []byte, mode os.FileMode) (string, string, string, error) {
	tmp, err := os.CreateTemp(filepath.Dir(targetPath), "."+filepath.Base(targetPath)+".patty-update-stage-*")
	if err != nil {
		return "", "", "", err
	}
	path := tmp.Name()
...
func quarantineExistingAppBundleUpdateBackup(tx *UpdateTransaction) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("resolve current Patty Code executable: %w", err)
	}
	...
	if err != nil || rel == "." || rel == ".." || strings.HasPrefix(rel, ".."+string(filepath.Separator)) {
		return "", "", fmt.Errorf("existing handoff backup is outside the current Patty Code installation")
	}
...
// a path merely because its name resembles a patty quarantine.
...
func writePendingUpdate(tx *UpdateTransaction, createOnly bool) error {
	...
	path := PendingUpdatePath()
	if path == "" {
		return fmt.Errorf("pending update: Patty Code state directory is unavailable")
	}
...
func classifyPendingUpdate() (pendingUpdateDisposition, *UpdateTransaction, error) {
	path := PendingUpdatePath()
	if path == "" {
		return pendingUpdateNone, nil, fmt.Errorf("Patty Code state directory is unavailable")
	}
...
// recovery needs regardless of where Patty Code is installed: which release it
...
func quarantinePendingUpdate(reason string) (string, error) {
	path := PendingUpdatePath()
	if path == "" {
		return "", fmt.Errorf("Patty Code state directory is unavailable")
	}
...
func rollbackPendingUpdateMatchingLocked(
	...
	// PATTY_HOME profiles cannot quarantine or restore the same binaries
	...
	case "app-bundle":
		...
		if _, statErr := os.Lstat(tx.TargetPath); statErr == nil {
			...
			var retainErr error
			failed, retainErr = retainUpdateRollbackNode(tx.TargetPath, "patty-failed")
			if retainErr != nil {
...
func restoreReleaseUnit(
	...
	for i, f := range files {
		aside := f.TargetPath + ".patty-rollback-aside"
...
func allowedUpdateTargetBase(base string, primary bool) bool {
	...
	case "patty-desktop", "patty-desktop.exe", "patcode-desktop", "patcode-desktop.exe":
		return primary
	case "patty", "patty.exe", "patcode", "patcode.exe", "patty-code", "patty-code.exe":
		return !primary
	case "patty-guard", "patty-guard.exe", "patty-launcher", "patty-launcher.exe",
		"patty-update-helper", "patty-update-helper.exe", "patty-cli", "patty-cli.exe",
		"patcode-guard", "patcode-guard.exe", "patcode-launcher", "patcode-launcher.exe",
		"patcode-update-helper", "patcode-update-helper.exe", "patcode-cli", "patcode-cli.exe",
		"patty-code-guard", "patty-code-guard.exe", "patty-code-launcher", "patty-code-launcher.exe",
		"patty-code-update-helper", "patty-code-update-helper.exe", "patty-code-cli", "patty-code-cli.exe":
		return !primary
	default:
		return false
	}
...
func validateUpdateTransactionForLauncher(tx *UpdateTransaction, launcher string) error {
	...
	case "file":
		if !allowedUpdateTargetBase(filepath.Base(tx.TargetPath), true) {
			return fmt.Errorf("pending update target is not a patty executable")
		}
	...
	case "app-bundle":
		if !strings.HasSuffix(strings.ToLower(tx.TargetPath), ".app") || tx.BackupPath != tx.TargetPath+".patty-update-backup" {
...
func validateAppBundleHandoffMetadata(tx *UpdateTransaction) error {
	...
	if !strings.HasPrefix(stagingBase, "patty-mac-update-") {
...
func validateOrphanedAppBundleBackupMetadata(tx *UpdateTransaction) error {
	...
	prefix := filepath.Base(tx.BackupPath) + ".patty-orphaned-"
