...
func TestIsolateUserStateRedirectsAndRestoresCallerEnvironment(t *testing.T) {
	...
	callerEnvironment := map[string]string{
		...
		"PATTY_HOME":       filepath.Join(callerHome, "explicit-patty-home"),
		"PATTY_STATE_HOME": filepath.Join(callerHome, "caller-state"),
		"PATTY_CACHE_HOME": filepath.Join(callerHome, "caller-cache"),
	}
