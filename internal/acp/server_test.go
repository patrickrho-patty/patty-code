...
func startServer(t *testing.T, factory Factory) (*rpcClient, func()) {
	...
	go func() {
		_ = Serve(context.Background(), inR, outW, factory, AgentInfo{Name: "patty-test", Version: "0"})
		close(done)
	}()
...
func TestServeLifecycle(t *testing.T) {
	...
	var extensions struct {
		AgentCapabilities struct {
			Meta map[string]PattyCodeExtensionCapabilities `json:"_meta"`
		} `json:"agentCapabilities"`
	}
	if err := json.Unmarshal(initResp.Result, &extensions); err != nil {
		t.Fatalf("initialize extensions: %v", err)
	}
	steer := extensions.AgentCapabilities.Meta["patty.io"].SessionSteer
	...
	for _, method := range []string{sessionStatusMethod, sessionStatusUpdateMethod} {
		...
		if !ok || capability["schemaVersion"] != float64(pattyStatusSchemaVersion) {
			t.Errorf("%s capability = %#v, want schemaVersion %d", method, ir.AgentCapabilities.Meta[method], pattyStatusSchemaVersion)
		}
	}
	if len(ir.AuthMethods) != 1 || ir.AuthMethods[0].ID != "patty-setup" || ir.AuthMethods[0].Type != "terminal" {
		t.Fatalf("authMethods = %+v, want terminal patcode setup", ir.AuthMethods)
	}
	if len(ir.AuthMethods[0].Args) != 1 || ir.AuthMethods[0].Args[0] != "setup" {
		t.Fatalf("auth args = %+v, want [setup]", ir.AuthMethods[0].Args)
	}

	authResp := client.call(t, "authenticate", AuthenticateParams{MethodID: "patty-setup"})
