...
func TestSessionSidecarLayout(t *testing.T) {
	const p = "/home/u/.patty/sessions/abc.jsonl"
	cases := []struct {
		name string
		got  string
		want string
		...
		{"goal-state", SessionGoalState(p), "/home/u/.patty/sessions/abc.goal-state.json"},
		{"event-log", SessionEventLog(p), "/home/u/.patty/sessions/abc.events.jsonl"},
		{"event-log-damaged", SessionEventLogDamaged(p), "/home/u/.patty/sessions/abc.events.jsonl.damaged"},
		{"event-index", SessionEventIndex(p), "/home/u/.patty/sessions/abc.event-index.json"},
		{"conflict-log", SessionConflictLog(p), "/home/u/.patty/sessions/abc.conflicts.jsonl"},
		{"lock", SessionLockFile(p), p + ".lock"},
		{"lease-lock", SessionLeaseLock(p), p + ".lease.lock"},
		{"lease-info", SessionLeaseInfo(p), p + ".lease.json"},
		{"checkpoint", SessionCheckpointDir(p), "/home/u/.patty/sessions/abc.ckpt"},
		{"jobs", SessionJobsDir(p), "/home/u/.patty/sessions/abc.jobs"},
		{"cleanup-pending", SessionCleanupPending(p), "/home/u/.patty/sessions/abc.cleanup-pending.json"},
		{"context", SessionContext(p), "/home/u/.patty/sessions/abc.context.json"},
	}
	for _, c := range cases {
		if c.got != c.want {
...
func TestSessionSidecarFiles(t *testing.T) {
	...
	want := []string{
		p + ".meta",
		"/home/u/.patty/sessions/abc.goal-state.json",
		"/home/u/.patty/sessions/abc.events.jsonl",
		"/home/u/.patty/sessions/abc.events.jsonl.damaged",
		"/home/u/.patty/sessions/abc.event-index.json",
		"/home/u/.patty/sessions/abc.conflicts.jsonl",
		"/home/u/.patty/sessions/abc.recovery.json",
		"/home/u/.patty/sessions/abc.context.json",
	}
	if len(got) != len(want) {
