// Package remote is the SSH transport for patty's remote module: host
// resolution ([remote] config + ~/.ssh/config), authentication, host-key
// verification (system known_hosts read-only + a patty-managed TOFU file),
...
// through this package — remote workspaces are driven by a `patcode serve`
