...
func TestBashMergesLoginShellPath(t *testing.T) {
	...
	probe := filepath.Join(bin, "patty-path-probe")
	...
	args, _ := json.Marshal(map[string]string{"command": "patty-path-probe"})
...
func TestBashCommandEnvFiltersSensitiveKeysWhenEnabled(t *testing.T) {
	...
	t.Setenv("PATTY_TEST_VISIBLE", "ok")
	...
	if !strings.Contains(env, "PATTY_TEST_VISIBLE=ok") {
...
func TestParseShellPATH(t *testing.T) {
	const marker = "__PATTY_BASH_PATH__="
	cases := []struct {
		name string
		out  string
		want string
...
func TestRunShellPATHCommandFiltersEnvWhenEnabled(t *testing.T) {
	...
	out := runShellPATHCommand(context.Background(), "/bin/sh", []string{"-c", `printf 'tok=%s' "${PATTY_TEST_SECRET_TOKEN:-none}"`})
