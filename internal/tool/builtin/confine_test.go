...
func TestManagedConfigWriteFailsClosedWithoutApprover(t *testing.T) {
	...
	cfg := config.Default()
	managed := NewManagedConfigPaths(config.PattyCodeManagedConfigPaths())
...
func TestManagedConfigWriteGatedOnApprover(t *testing.T) {
	...
	decline := &stubConfigWriteApprover{allow: false, reason: "the user declined this Patty Code config write"}
	...
	// Even with an always-allowing approver, non-config files in the patty
	...
	for _, target := range []string{
		...
		filepath.Join(home, ".patty", ".env"),
		filepath.Join(home, ".patty", "settings.json"),
		filepath.Join(home, ".patty", "skills", "evil", "SKILL.md"),
	} {
...
func TestBashSandboxConfinement(t *testing.T) {
	...
	work, err := os.MkdirTemp(home, ".patty-bashsb-*")
	if err != nil {
	...
	outPath := filepath.Join(home, ".patty-bashsb-escape.txt")
