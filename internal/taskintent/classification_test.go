...
func TestDeliveryClassificationMatrix(t *testing.T) {
	tests := []struct {
		...
		{"chinese how to install", "플러그인을 어떻게 설치하나요", false, false},
		{"chinese how to modify", "WPS 설정을 어떻게 수정하나요", false, false},
		...
		{"negated chinese", "코드를 고치지 마세요", false, false},
		{"negated inspection", "Explain why it fails. Do not inspect or change files.", false, false},
		{"negated chinese inspection", "왜 실패했는지 설명해 주고, 파일은 건드리지 마세요", false, false},
		...
		{"chinese thanks then update", "고마워요, 계속해서 설정을 수정해 줘", true, true},
		...
		{"chinese polite fix before advice", "왜 실패했는지, 수정해 줘", true, true},
		{"chinese advice then fix", "왜 실패했는지, 수정해 줘", true, true},
		...
		{"chinese reset", "의존성을 설치할 수 없어서, 기존 설정에서 수정해 줘", true, true},
		...
		{"chinese deferred conversation token", "ORBIT-42를 기억하고, 다음 라운드에서 답변해 줘", false, false},
		{"deferred token with durable negation", "Remember ORBIT-42 for the next turn. Do not save it permanently.", false, false},
		{"chinese deferred token with durable negation", "ORBIT-42를 기억하고, 다음 라운드에서 답변해 줘. 파일이나 장기 기억에 기록하지 마세요.", false, false},
		...
		{"chinese durable memory", "ORBIT-42를 영구히 기억해요, 세션 간에도 보관해 줘", true, true},
