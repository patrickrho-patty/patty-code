...
// MCPActivationFile is the on-disk shape of $PATTY_HOME/mcp-activation.json.
...
// MCPActivationPath returns the durable activation file under patty home.
func MCPActivationPath(pattyHome string) string {
	return filepath.Join(strings.TrimSpace(pattyHome), mcpActivationFilename)
}

// NewMCPActivationStore opens the activation store for pattyHome.
func NewMCPActivationStore(pattyHome string) *MCPActivationStore {
	return &MCPActivationStore{path: MCPActivationPath(pattyHome)}
}

// DefaultMCPActivationStore uses the process patty home.
func DefaultMCPActivationStore() *MCPActivationStore {
	return NewMCPActivationStore(PattyHomeDir())
}
...
// independent store instances and separate Patty Code processes. Atomic rename
