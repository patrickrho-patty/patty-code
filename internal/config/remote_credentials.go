...
// RemotePasswordCredentialEnvName returns the patty-owned credential slot
...
// RemotePassphraseCredentialEnvName returns the patty-owned credential slot
...
func remoteCredentialEnvName(hostID, kind string) string {
	...
	return fmt.Sprintf("PATTY_REMOTE_%X_%s", sum[:8], kind)
}

// IsGeneratedRemoteCredential reports whether key is a patty-owned slot
...
// EditUserConfigWithCredentials updates config and its Patty Code-owned secret
