...
func TestExpandVars(t *testing.T) {
	t.Setenv("PATTY_TEST_TOKEN", "sk-123")
	t.Setenv("PATTY_TEST_EMPTY", "")
...
func TestExpandedPlugin(t *testing.T) {
	t.Setenv("PATTY_TEST_KEY", "secret")
	e := PluginEntry{
		Name:    "x",
		Type:    "http",
		URL:     "https://api/${PATTY_TEST_MISSING:-v1}",
		Args:    []string{"--token", "${PATTY_TEST_KEY}"},
		Env:     map[string]string{"K": "${PATTY_TEST_KEY}"},
		Headers: map[string]string{"Authorization": "Bearer ${PATTY_TEST_KEY}"},
	}
	...
	if e.Headers["Authorization"] != "Bearer ${PATTY_TEST_KEY}" {
...
func TestForbidReadRootsForRootResolvesRelativePathsAndScopedEnv(t *testing.T) {
	root := t.TempDir()
	cfg := Default()
	cfg.setExpansionEnv(map[string]string{"PATTY_TEST_SECRET_DIR": "from-dotenv"})
	cfg.Sandbox.ForbidRead = []string{
		"relative-secret",
		"${PATTY_TEST_SECRET_DIR}",
