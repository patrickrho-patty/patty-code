...
func isolateUserConfigHome(t *testing.T) string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	for _, key := range []string{"PATTY_HOME", "PATTY_STATE_HOME", "PATTY_CACHE_HOME"} {
		t.Setenv(key, "")
...
func setRuntimeGOOS(t *testing.T, goos string) {
	t.Helper()
	t.Setenv("PATTY_TEST_GOOS", goos)
	old := runtimeGOOS
	runtimeGOOS = goos
	t.Cleanup(func() { runtimeGOOS = old })
}

func expectedDefaultPattyCodeHome(home string) string {
...
func TestUserConfigDisplayPathCollapsesHome(t *testing.T) {
	...
	if !strings.HasSuffix(got, "patty/config.toml") {
		t.Fatalf("display path = %q, want patty/config.toml suffix", got)
	}
...
func TestUserConfigPathUsesPattyCodeHome(t *testing.T) {
	home := isolateUserConfigHome(t)
	want := filepath.Join(expectedDefaultPattyCodeHome(home), "config.toml")
	if got := UserConfigPath(); filepath.Clean(got) != filepath.Clean(want) {
		t.Fatalf("UserConfigPath() = %q, want %q", got, want)
	}
}

func TestPattyCodeManagedConfigPathsAreConfigFilesOnly(t *testing.T) {
	home := isolateUserConfigHome(t)
	...
	paths := PattyCodeManagedConfigPaths()
	for _, want := range []string{
		filepath.Join(home, "AppData", "Roaming", "patty", "config.toml"),
		filepath.Join(home, ".patty", "config.json"),
	} {
		found := false
	...
	// the sensitive Patty Code-home siblings (credentials, hooks, skills,
	...
	for _, got := range paths {
		...
		for _, forbidden := range []string{
			...
			PattyHomeDir(),
			UserCredentialsPath(),
			filepath.Join(PattyHomeDir(), "settings.json"),
			filepath.Join(PattyHomeDir(), "skills"),
			filepath.Join(PattyHomeDir(), "sessions"),
		} {
...
func TestUserConfigPathHonorsPattyCodeHome(t *testing.T) {
	home := isolateUserConfigHome(t)
	custom := filepath.Join(home, "custom-home")
	t.Setenv("PATTY_HOME", custom)
...
func TestLoadForRootUsesWindowsHomeFallbackWhenConfigDirUnavailable(t *testing.T) {
	...
	configPath := filepath.Join(home, "AppData", "Roaming", "patty", "config.toml")
...
func TestRenderTOMLUsesKoreanFirstLocaleExamples(t *testing.T) {
	...
	for _, want := range []string{`language      = "ko-KR"`, `language = "ko-KR"`, `reasoning_language = "ko-KR"`} {
		if !strings.Contains(rendered, want) {
			t.Fatalf("rendered config missing Korean-first example %q:\n%s", want, rendered)
		}
	}
}
...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Agent.ReasoningLanguage = "ko-KR"
	...
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "patty-plugin-example"},
	...
	if len(got.Bot.Connections) != 1 || got.Bot.Connections[0].Model != "deepseek-pro" || got.Bot.Connections[0].WorkspaceRoot != "/tmp/patty-bot" {
...
func TestScopedRenderSeparatesUserAndProjectConfig(t *testing.T) {
	...
	for _, want := range []string{"config_version = 5", "[desktop]", `currency = "KRW"`, `theme = "dark"`, `terminal_theme = "auto"`, `close_behavior = "background"`, `status_bar_style = "text"`, `default_tool_approval_mode = "auto"`, `check_updates = false`, `recovery_model = "deepseek-pro"`, "[notifications]", "[tools.shell]"} {
...
func TestRenderTOMLRoundTripsPerModelPrices(t *testing.T) {
	...
	orig.Providers = []ProviderEntry{{
		...
		Prices:    DeepSeekV4PricesForCurrency("KRW"),
	}}
...
func TestRenderTOMLRoundTripsProviderHeadersAndModelOverrides(t *testing.T) {
	...
	rendered := RenderTOML(orig)
	if !strings.Contains(rendered, `headers     = { HTTP-Referer = "https://app.example", X-Title = "Patty Code" }`) {
	...
	if p.Headers["HTTP-Referer"] != "https://app.example" || p.Headers["X-Title"] != "Patty Code" {
...
func TestIsolatedHomeDirReturnsCleanPath(t *testing.T) {
	raw := filepath.Join(t.TempDir(), "isolated-patty")
	t.Setenv("PATTY_HOME", raw)
	got := IsolatedHomeDir()
...
func TestLegacyOSSupportDirEmptyWhenIsolated(t *testing.T) {
	isolateUserConfigHome(t)
	t.Setenv("PATTY_HOME", filepath.Join(t.TempDir(), "isolated-home"))
...
func TestCacheDirHonorsPattyCodeHome(t *testing.T) {
	home := t.TempDir()
...
func TestCacheDirHonorsPattyCodeCacheHomeOverPattyCodeHome(t *testing.T) {
	home := t.TempDir()
	cacheHome := filepath.Join(home, "custom-cache")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("PATTY_HOME", filepath.Join(home, "isolated-home"))
	t.Setenv("PATTY_CACHE_HOME", cacheHome)

	got := CacheDir()
	want := cacheHome
	if filepath.Clean(got) != filepath.Clean(want) {
		t.Fatalf("CacheDir() = %q, want %q (PATTY_CACHE_HOME must win)", got, want)
	}
}
...
func TestUserConfigLoadPathNoLegacyFallbackWhenIsolated(t *testing.T) {
	...
	productionHome := expectedDefaultPattyCodeHome(home)
...
func TestMigrateLegacyIfNeededSkipsWhenIsolated(t *testing.T) {
	...
	legacyDir := filepath.Join(home, ".patty")
...
func TestProjectConfigCannotOverrideSecrets(t *testing.T) {
	...
	if cfg.Secrets.FilterSubprocessEnv {
		t.Error("project patty.toml enabled filter_subprocess_env; [secrets] must stay user-global")
	}
	if cfg.Secrets.ProtectSensitiveFiles {
		t.Error("project patty.toml enabled protect_sensitive_files; [secrets] must stay user-global")
	}
}
