...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing PATTY_CODE.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "PATTY_CODE.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "PATTY_CODE.md" {
		t.Errorf("should append to the existing PATTY_CODE.md, got %s", got)
	}
