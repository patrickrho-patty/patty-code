...
// PATTY_BOOT_FAKE_SIDECAR=1 and speaks Extension Protocol v2 over
// stdin/stdout. PATTY_BOOT_FAKE_INIT_RESULT overrides the initialize
// result; PATTY_BOOT_FAKE_MODE=ignore_shutdown keeps the process alive
// through extension/shutdown. Intercept steering for the dispatch tests:
//
//	PATTY_BOOT_FAKE_BLOCK_EVENT      answer block at this event
//	PATTY_BOOT_FAKE_INVALID_EVENT    answer a DTO-violating replace at this event
//	PATTY_BOOT_FAKE_REPLACE_PROMPT   answer system_prompt.build replace with this prompt
//	PATTY_BOOT_FAKE_REPLACE_INPUT    answer input.receive replace with this text
//	PATTY_BOOT_FAKE_EVENT_LOG        append one "event payload" line per extension/event
//
// Provider steering for the stage 7 adapter tests:
//
//	PATTY_BOOT_FAKE_PLUGIN_NAME      the installed plugin name (provider ref namespace)
//	PATTY_BOOT_FAKE_PROVIDER         when "1", declare plugin/<name>/fake/x and serve
//	                                    catalog/stream/open/stream/cancel with a fixed
//	                                    two-chunk completion plus usage
//
// UI steering for the stage 8a hub tests:
//
//	PATTY_BOOT_FAKE_UI_PUBLISH       when "1", publish one credential-bearing
//	                                    status surface through host/ui/publish after
//	                                    the handshake completes
//
// Process-lifecycle steering for the failure-cleanup tests:
//
//	PATTY_BOOT_FAKE_PID_FILE         write the sidecar PID to this file on start,
//	                                    so the parent can poll for a leaked process
//	PATTY_BOOT_FAKE_EXIT_IMMEDIATELY when "1", write the PID file (if set) and
//	                                    exit 0 at once — a sidecar that dies before
//	                                    answering the handshake
const (
	bootFakeEnvEnable          = "PATTY_BOOT_FAKE_SIDECAR"
	bootFakeEnvInitResult      = "PATTY_BOOT_FAKE_INIT_RESULT"
	bootFakeEnvMode            = "PATTY_BOOT_FAKE_MODE"
	bootFakeEnvBlockEvent      = "PATTY_BOOT_FAKE_BLOCK_EVENT"
	bootFakeEnvInvalidEvent    = "PATTY_BOOT_FAKE_INVALID_EVENT"
	bootFakeEnvReplacePrompt   = "PATTY_BOOT_FAKE_REPLACE_PROMPT"
	bootFakeEnvReplaceInput    = "PATTY_BOOT_FAKE_REPLACE_INPUT"
	bootFakeEnvEventLog        = "PATTY_BOOT_FAKE_EVENT_LOG"
	bootFakeEnvPluginName      = "PATTY_BOOT_FAKE_PLUGIN_NAME"
	bootFakeEnvProvider        = "PATTY_BOOT_FAKE_PROVIDER"
	bootFakeEnvUIPublish       = "PATTY_BOOT_FAKE_UI_PUBLISH"
	bootFakeEnvPIDFile         = "PATTY_BOOT_FAKE_PID_FILE"
	bootFakeEnvExitImmediately = "PATTY_BOOT_FAKE_EXIT_IMMEDIATELY"
)
...
func bootWithFakePlugin(t *testing.T, name string, runtime map[string]any) *BuildResult {
	...
	installBootFakePlugin(t, config.PattyHomeDir(), name, runtime)
...
func TestBootFailsWhenTwoRuntimesClaimOneSlot(t *testing.T) {
	...
	installBootFakePlugin(t, pattyHome, "claim-one", map[string]any{
		"replaces": []string{"system_prompt"},
	})
	installBootFakePlugin(t, pattyHome, "claim-two", map[string]any{
		"replaces": []string{"system_prompt"},
	})
...
func TestBootFailsWhenRequiredRuntimeFails(t *testing.T) {
	...
	t.Chdir(dir)
	writeRuntimeFixture(t, dir)
	installBootFakePlugin(t, config.PattyHomeDir(), "required-broken", map[string]any{
		"required": true,
...
func TestRebuildRetiresOldSidecars(t *testing.T) {
	...
	t.Chdir(dir)
	writeRuntimeFixture(t, dir)
	installBootFakePlugin(t, config.PattyHomeDir(), "rebuildplugin", map[string]any{})
