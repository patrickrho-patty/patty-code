...
// fakeProgressClock drives the mergers pacing deterministically: tests advance
// the clock instead of sleeping, and the mergers timer fires exactly when the
...
func TestSubagentProgressUtf8TailKeepsRuneBoundaries(t *testing.T) {
	delta := strings.Repeat("한", 4096) // 12 KiB of multi-byte pending text
	clock := newFakeProgressClock(time.Unix(0, 0))
	ch := make(chan event.Event, 64)
	trk := newTestTracker(t, clock, chanSink{ch: ch}, "task-1")
	trk.running()
...
func TestSubagentProgressTrimTruncationPropagates(t *testing.T) {
	...
	waitEvent(t, ch, "running status")
	delta := strings.Repeat("한", 4096) // 12 KiB per channel; the shared 8 KiB budget trims
