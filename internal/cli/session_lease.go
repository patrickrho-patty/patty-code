...
// sessionLeaseResumeRefusal is the startup-time refusal for `patty
// [--resume|--continue]` and `patcode run --resume/--continue`: it names the
...
func sessionLeaseResumeRefusal(err error) string {
	return control.SessionInUseMessage(err) +
		"; close the other patty window or process, or rerun with --copy to continue in a duplicated session"
}
