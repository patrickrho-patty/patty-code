...
func TestHookMachineListRedactsCommandsAndEnablesProjectHooks(t *testing.T) {
	root := t.TempDir()
	home := t.TempDir()
	projectSettings := filepath.Join(root, ".patty", "settings.json")
	...
	globalSettings := filepath.Join(home, ".patty", "settings.json")
...
// hook machine resolves global hooks from the platform patty home
// (config.PattyHomeDir → settings.json), not a doubled .patty segment
// (#7420). PATTY_HOME is set explicitly so the resolved path is
