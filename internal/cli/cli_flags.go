...
const resumePickerSentinel = "__patty_resume_picker__"
...
// the top-level flag run, i.e. before any "--" terminator. patty has no
// interactive -p, so its presence means the user wants one-shot print mode even
// when it trails other flags (`patcode --model X -p "task"`).
