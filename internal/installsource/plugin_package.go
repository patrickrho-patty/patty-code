...
func (t *installSourceTool) pluginPackageAction(req request, pkg pluginpkg.Package, source string) (action, error) {
	...
	root := ""
	if t.pattyHome != "" {
		root = pluginpkg.InstallRoot(t.pattyHome, name)
	}
	skills, commands, hooks, mcp := pkg.CapabilityCounts()
	agents := pkg.Inventory().Agents
	if pkg.ManifestKind != "patty" && skills+commands+hooks+mcp+len(agents) == 0 {
		return action{}, newErr(ErrNoCompatibleCapabilities, "plugin %q has no Patty Code-compatible capabilities; skipped: %v", name, pkg.Compatibility.Skipped)
	}
	...
	if hooks > 0 {
		...
		a.RiskReasons = append(a.RiskReasons, "registers shell hooks that execute during patty sessions")
	}
	if mcp > 0 {
		a.RiskLevel = RiskHigh
	...
	if a.Runtime != nil {
		a.RiskLevel = RiskHigh
		a.RiskReasons = append(a.RiskReasons, "FULL TRUST: declares a runtime process ("+pluginpkg.RuntimeCommandLine(pkg.Manifest.Runtime)+") that runs inside Patty Code — it can read the full session and environment, bypass permissions, and operate this machine directly")
	}
...
func (t *installSourceTool) applyInstallPluginPackage(ctx context.Context, req request, act *action) error {
	if t.pattyHome == "" {
		return newErr(ErrSourceUnreadable, "plugin install requires a patty home directory")
	}
	if !pluginpkg.IsValidName(act.Name) {
		return newErr(ErrInvalidManifest, "invalid plugin name %q", act.Name)
	}
	target := pluginpkg.InstallRoot(t.pattyHome, act.Name)
	...
	if pkg.ManifestKind != "patty" {
		...
		if skills+commands+hooks+mcp+pkg.AgentCount() == 0 {
			return newErr(ErrInvalidManifest, "plugin %q no longer has any Patty Code-compatible capabilities", act.Name)
		}
	}
	...
	installed := pluginpkg.InstalledPlugin{
		Name:         act.Name,
		Source:       act.Source,
		Root:         pluginpkg.RelativeRoot(t.pattyHome, target),
		Version:      pkg.Manifest.Version,
	...
	if err := pluginpkg.Upsert(t.pattyHome, installed); err != nil {
		return err
	}
	act.Target = target
...
func (t *installSourceTool) preparePluginSource(ctx context.Context, source, mode string) (string, string, func(), error) {
	...
	if isURL(source) {
		...
		tmp, err := os.MkdirTemp("", "patty-plugin-*")
		if err != nil {
			return "", "", func() {}, err
		}
...
func (t *installSourceTool) applyRemovePluginPackage(_ request, act *action) error {
	installed, ok, err := pluginpkg.Remove(t.pattyHome, act.Name)
	if err != nil || !ok {
		return err
	}
	root := pluginpkg.ResolveRoot(t.pattyHome, installed.Root)
	...
	pluginsDir := pluginpkg.PluginsDir(t.pattyHome)
